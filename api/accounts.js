...
module.exports = async (req, res) => {
  ...
  try {
    const userId = await getUserId(req.method === 'GET' ? req.query.initData : req.body?.initData);
    if (req.method === 'GET') {
      ...
      const { data, error } = await supabase.from('accounts').insert({ user_id: userId, name, type: type || 'expense', balance: 0 }).select().single();
      if (error) throw error;
      return res.json(data);
    }
