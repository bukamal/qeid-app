...
function showAddItemModal() {
  ...
  showFormModal({
    title: 'إضافة مادة جديدة',                       fields: [
      { id: 'name', label: 'اسم المادة', placeholder: 'مثال: حبر طابعة' },                              { id: 'category_id', label: 'التصنيف', type: 'select', options: `<option value="">بدون تصنيف</option>${catOpts}` },
      { id: 'item_type', label: 'نوع المادة', type: 'select', options: '<option value="مخزون">مخزون</option><option value="منتج نهائي">منتج نهائي</option><option value="خدمة">خدمة</option>' },          { id: 'unit', label: 'وحدة القياس', placeholder: 'قطعة، صندوق، كرتونة...' },                      { id: 'purchase_price', label: 'سعر الشراء', type: 'number', placeholder: '0.00' },
      { id: 'selling_price', label: 'سعر البيع', type: 'number', placeholder: '0.00' }
    ],                                               onSave: async (values) => {
      const payload = { ...values, category_id: values.category_id || null, purchase_price: parseFloat(values.purchase_price) || 0, selling_price: parseFloat(values.selling_price) || 0 };               if (itemsCache.some(i => i.name.toLowerCase() === payload.name.toLowerCase())) throw new Error('توجد مادة بنفس الاسم');
...
function showAddItemModal() {
  ...
  const unitOpts = unitsCache.map(u => `<option value="${u.id}">${u.name}</option>`).join('');
                                                   const body = `                                     <div class="form-group"><label class="form-label">اسم المادة</label><input class="input" id="fm-name" type="text" placeholder="مثال: حبر طابعة"></div>
    ...
    <div class="form-group"><label class="form-label">الوحدة الأساسية <span style="color:var(--text-muted);font-size:12px;">(وحدة المخزون)</span></label><select class="select" id="fm-base-unit"><option value="">اختر وحدة...</option>${unitOpts}</select></div>                                        <div class="form-group"><label class="form-label" style="font-size:12px;color:var(--text-muted);">أو أضف وحدة جديدة</label><div style="display:flex;gap:8px;"><input class="input" id="fm-new-unit" type="text" placeholder="اسم الوحدة..." style="flex:1;"><button class="btn btn-secondary" id="btn-quick-unit" type="button" style="width:auto;padding:0 14px;">${ICONS.plus}</button></div></div>
  ...
  let conversions = [];
  const convList = modal.element.querySelector('#conversions-list');
  const refreshConversions = () => {                 const baseName = modal.element.querySelector('#fm-base-unit option:checked')?.textContent || 'وحدة';
    convList.innerHTML = conversions.map((c, idx) => `
      <div style="display:flex;justify-content:space-between;align-items:center;background:var(--bg);padding:8px 12px;border-radius:8px;">                 <span style="font-weight:700;">1 ${c.unit_name} = ${c.conversion_factor} ${baseName}</span>
        <button class="btn btn-ghost btn-sm" data-idx="${idx}" style="color:var(--danger);padding:4px;">${ICONS.trash}</button>
      </div>`).join('');
    convList.querySelectorAll('button[data-idx]').forEach(b => { b.onclick = () => { conversions.splice(parseInt(b.dataset.idx), 1); refreshConversions(); }; });
  };

  modal.element.querySelector('#btn-add-conv').onclick = () => {                                      const unitSel = modal.element.querySelector('#conv-unit');                                        const factor = parseFloat(modal.element.querySelector('#conv-factor').value);                     const unitId = unitSel.value, unitName = unitSel.options[unitSel.selectedIndex]?.textContent;     const baseUnitId = modal.element.querySelector('#fm-base-unit').value;
    if (!unitId) return showToast('اختر الوحدة الفرعية', 'warning');
    if (!factor || factor <= 0) return showToast('أدخل معامل تحويل صحيح', 'warning');                 if (unitId === baseUnitId) return showToast('لا يمكن إضافة الوحدة الأساسية كوحدة تحويل', 'warning');                                               if (conversions.some(c => c.unit_id == unitId)) return showToast('الوحدة مضافة مسبقاً', 'warning');                                                 conversions.push({ unit_id: parseInt(unitId), unit_name: unitName, conversion_factor: factor });
    modal.element.querySelector('#conv-factor').value = ''; unitSel.value = '';
    refreshConversions();
  };                                                                                                const quickAdd = async (btnId, inputId, selectId, cacheArr, type, msg) => {                         const btn = modal.element.querySelector(btnId), input = modal.element.querySelector(inputId), select = modal.element.querySelector(selectId);
    btn.onclick = async () => {
      const name = input.value.trim();
      if (!name) return showToast('أدخل الاسم أولاً', 'warning');                                        if (cacheArr.some(x => x.name.toLowerCase() === name.toLowerCase())) return showToast('الاسم موجود مسبقاً', 'warning');                             btn.disabled = true; btn.innerHTML = `<span class="loader-inline"></span>`;                       try {                                              const res = await apiCall(`/definitions?type=${type}`, 'POST', { type, name });                   const newName = res?.name || name, newId = res?.id || res?.data?.id;
        if (!newId) throw new Error('خطأ في الاستجابة');
        cacheArr.push({ id: newId, name: newName });                                                      [select, modal.element.querySelector('#conv-unit')].forEach(s => {                                  if (s && ![...s.options].some(o => o.value == newId)) { const o = document.createElement('option'); o.value = newId; o.textContent = newName; s.appendChild(o); }
        });
        select.value = newId; input.value = '';
        showToast(msg, 'success');
      } catch (e) { showToast(e.message, 'error'); }
  ...
  modal.element.querySelector('#fm-save').onclick = async () => {
    ...
    try {                                              const btn = modal.element.querySelector('#fm-save');                                              btn.disabled = true; btn.innerHTML = `<span class="loader-inline"></span> جاري الحفظ...`;         await apiCall('/items', 'POST', values);
      modal.close(); showToast('تم الحفظ بنجاح', 'success'); loadItems();
    } catch (e) { showToast(e.message, 'error'); btn.disabled = false; btn.innerHTML = `${ICONS.check} حفظ`; }
  };
}
