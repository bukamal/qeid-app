...
async function apiCall(endpoint, method = 'GET', body = {}) {
  ...
  const res = await fetch(url, options); const json = await res.json().catch(() => ({}));
  ...
  if (method === 'POST' || method === 'PUT' || method === 'DELETE') {
    ...
    else if (base === 'categories') categoriesCache = await apiCall('/categories','GET');
  }
  return json;
}
...
async function loadDashboard() {
  try {
    const [monthly, daily, summary] = await Promise.all([apiCall('/reports?type=monthly_summary','GET'), apiCall('/reports?type=daily_profit','GET'), apiCall('/summary','GET')]);
    ...
    setTimeout(()=>{
      const ctx1=document.getElementById('incomeChart'); if(ctx1) new Chart(ctx1,{type:'doughnut',data:{labels:['مبيعات','مشتريات'],datasets:[{data:[totalSales,totalPurchases],backgroundColor:['#10b981','#f59e0b'],borderColor:'#fff',borderWidth:2}]},options:{responsive:true,plugins:{legend:{position:'bottom'}}}});
      const ctx2=document.getElementById('paymentsChart'); if(ctx2&&monthly) new Chart(ctx2,{type:'bar',data:{labels:monthly.labels,datasets:[{label:'وارد',data:monthly.payments_in,backgroundColor:'#10b981'},{label:'منصرف',data:monthly.payments_out,backgroundColor:'#ef4444'}]},options:{responsive:true,plugins:{legend:{position:'bottom'}},scales:{y:{beginAtZero:true}}}});
      const ctx3=document.getElementById('profitChart'); if(ctx3&&daily){const pts=daily.dates.map((ds,i)=>({x:new Date(ds+'T00:00:00'),y:daily.profits[i]}));new Chart(ctx3,{type:'line',data:{datasets:[{label:'صافي الربح اليومي',data:pts,borderColor:'#3b82f6',backgroundColor:'rgba(59,130,246,0.1)',fill:true,pointRadius:3}]},options:{responsive:true,plugins:{legend:{position:'bottom'}},scales:{x:{type:'time',time:{unit:'day',displayFormats:{day:'yyyy-MM-dd'}},title:{display:true,text:'التاريخ'}},y:{beginAtZero:true,title:{display:true,text:'الربح'}}}}})}}
    },100);
...
function renderFilteredItems() {
  ...
  document.getElementById('items-list').innerHTML = filtered.map(item=>{
    const purchaseQty = invoicesCache.filter(inv=>inv.type==='purchase').flatMap(inv=>inv.invoice_lines||[]).filter(l=>l.item_id===item.id).reduce((s,l)=>s+(parseFloat(l.quantity)||0),0);
    const saleQty = invoicesCache.filter(inv=>inv.type==='sale').flatMap(inv=>inv.invoice_lines||[]).filter(l=>l.item_id===item.id).reduce((s,l)=>s+(parseFloat(l.quantity)||0),0);
    const available = purchaseQty - saleQty;
    const totalValue = available * (parseFloat(item.purchase_price)||0);
    return `<div class="card item-card" onclick="showItemDetailModal(${item.id})"><strong>${item.name}</strong><div style="display:flex;justify-content:space-between;margin-top:8px;font-size:0.9em;"><span>🛒 مشترى: ${purchaseQty}</span><span>💰 مباع: ${saleQty}</span><span>📦 متوفر: ${available}</span><span>💵 القيمة: ${totalValue.toFixed(2)}</span></div></div>`;
  }).join('');
}
...
function attachInvoiceEvents(invoiceType) {
  ...
  function autoFillPrice(sel,pr) {
    ...
    if(item){pr.value=(invoiceType==='sale'?item.selling_price:item.purchase_price)||0;const row=sel.closest('.line-row');const qty=row.querySelector('.qty-input'),tot=row.querySelector('.total-input');if(qty&&tot)tot.value=Math.round((parseFloat(qty.value)||0)*(parseFloat(pr.value)||0));}
  }
...
function renderFilteredInvoices() {
  ...
  document.getElementById('invoices-list').innerHTML = filtered.map(inv=>`
    <div class="card">
...
function showAddCategoryModal() {
  ...
  document.body.appendChild(overlay);
  document.getElementById('btn-save-cat').onclick=async()=>{const n=document.getElementById('cat-name').value.trim(); if(!n) return alert('الاسم مطلوب'); try{await apiCall('/categories','POST',{name:n}); document.body.removeChild(overlay); loadCategories();}catch(e){alert('خطأ: '+e.message);}};
