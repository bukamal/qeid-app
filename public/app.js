...
function showAddItemModal() {
  ...
  document.getElementById('btn-add-cat-in-modal').onclick = async () => {
    ...
    try {
      const nc = await apiCall('/categories', 'POST', { name: cn }); const sel = document.getElementById('add-item-category');
      const opt = document.createElement('option'); opt.value = nc.id; opt.textContent = nc.name; sel.appendChild(opt); sel.value = nc.id;
      document.getElementById('add-item-new-category').value = ''; categoriesCache.push(nc);
    } catch (e) { alert('خطأ: '+e.message); }
  };
  document.getElementById('save-add-item').onclick = async () => {
    const n = document.getElementById('add-item-name').value.trim(); if (!n) return alert('اسم المادة مطلوب');
    if (itemsCache.some(i => i.name.toLowerCase() === n.toLowerCase())) return alert('توجد مادة بنفس الاسم');
...
function updateInvoiceRemoveButtons() {
  document.querySelectorAll('#inv-lines-container .line-row').forEach(row => {
    const btn = row.querySelector('.btn-remove-line'); if (btn) btn.style.display = document.querySelectorAll('#inv-lines-container .line-row').length > 1 ? 'inline-block' : 'none';
  });
}
function attachInvoiceEvents(invoiceType) {
  function isItemDuplicate(id, cur) {
    if (!id) return false; let found = false;
    document.querySelectorAll('#inv-lines-container .line-row').forEach(r => { if (r === cur) return; const s = r.querySelector('.item-select'); if (s && s.value === id) found = true; });
    return found;
  }
  function autoFillPrice(sel, pr) {
    ...
    if (item) {
      ...
      const row = sel.closest('.line-row'); const qty = row.querySelector('.qty-input'); const tot = row.querySelector('.total-input');
  ...
  document.getElementById('btn-add-inv-line')?.addEventListener('click', () => {
    ...
    container.appendChild(line); updateInvoiceRemoveButtons(); attachLineEvents(line);
  ...
  function attachLineEvents(row) {
    const qty = row.querySelector('.qty-input'), pr = row.querySelector('.price-input'), tot = row.querySelector('.total-input');
  ...
  document.getElementById('btn-save-invoice')?.addEventListener('click', async () => {
    ...
    if (type === 'sale') cust = entity === 'cash' ? null : entity; else supp = entity === 'cash' ? null : entity;
    ...
    document.querySelectorAll('#inv-lines-container .line-row').forEach(row => {
      ...
      if (id) { if (ids.includes(id)) dup = true; ids.push(id); }
...
async function loadInvoiceFormByType(type) {
  try {
    ...
    document.getElementById('tab-content').innerHTML = `
      <div class="card"><h3>فاتورة ${type==='sale'?'مبيعات':'مشتريات'} جديدة</h3>
      ...
      <h4>البنود</h4><div id="inv-lines-container"><div class="line-row"><select class="input-field item-select"><option value="">اختر مادة</option>${items.map(i => `<option value="${i.id}">${i.name}</option>`).join('')}</select><input type="number" step="any" placeholder="الكمية" class="input-field qty-input" /><input type="number" step="0.01" placeholder="السعر" class="input-field price-input" /><input type="number" step="0.01" placeholder="الإجمالي" class="input-field total-input" readonly /><button class="btn-remove-line btn-secondary" style="display:none">✕</button></div></div>
      <button id="btn-add-inv-line" class="btn-secondary">+ بند</button>
      <h4>المدفوعات</h4><input id="inv-paid" type="number" step="0.01" placeholder="المبلغ المدفوع" class="input-field" value="0" />
      <button id="btn-save-invoice" class="btn-primary">حفظ الفاتورة</button></div>`;
...
async function loadInvoices() {
  try {
    const [invoices, customers, suppliers, items] = await Promise.all([apiCall('/invoices', 'GET'), apiCall('/customers', 'GET'), apiCall('/suppliers', 'GET'), apiCall('/items', 'GET')]);
...
function showEditInvoiceModal(invoice) {
  ...
  document.getElementById('save-invoice-edit').onclick = async () => {
    ...
    if (dup) return alert('لا يمكن تكرار نفس المادة');
    if (!lines.filter(l => l.item_id || l.quantity > 0).length) return alert('أضف بنداً');
...
function downloadPDF(invoice) {
  if (!invoice) return;
  const { jsPDF } = window.jspdf; if (!jsPDF) { alert('مكتبة PDF غير محملة.'); return; }
  ...
  doc.text(`${invoice.customer?.name?'العميل: '+invoice.customer.name:''} ${invoice.supplier?.name?'المورد: '+invoice.supplier.name:''}`, 105, 32, { align: 'center' });
  let y = 40;
  doc.text('المادة', 20, y); doc.text('الكمية', 70, y); doc.text('السعر', 110, y); doc.text('الإجمالي', 150, y); y += 6;
  invoice.invoice_lines?.forEach(l => { doc.text(l.item?.name||'-', 20, y); doc.text(String(l.quantity), 70, y); doc.text(String(l.unit_price), 110, y); doc.text(String(l.total), 150, y); y += 8; });
  y += 5;
  doc.text(`الإجمالي: ${invoice.total}`, 150, y, { align: 'right' }); y += 6;
  doc.text(`المدفوع: ${invoice.paid||0} | الباقي: ${invoice.balance||0}`, 150, y, { align: 'right' });
  if (invoice.notes) { y += 8; doc.text(invoice.notes, 20, y); }
  ...
  try {
    ...
    document.getElementById('btn-ledger').addEventListener('click', async () => {
      ...
      try {
        ...
        html += '</table>'; document.getElementById('ledger-result').innerHTML = html;
...
async function loadCustomerStatementForm() {
  try {
    ...
    document.getElementById('tab-content').innerHTML = `<div class="card"><button class="btn-secondary" onclick="loadReports()">🔙 رجوع</button><h3>كشف حساب عميل</h3><select id="stmt-cust" class="input-field">${opts}</select><button id="btn-stmt-cust" class="btn-primary">عرض الكشف</button><div id="stmt-result"></div></div>`;
    document.getElementById('btn-stmt-cust').addEventListener('click', async () => {
      ...
      try {
        ...
        let html = '<table class="report-table"><tr><th>التاريخ</th><th>الوصف</th><th>مدين</th><th>دائن</th><th>الرصيد</th></tr>';
        lines.forEach(l => html += `<tr><td>${l.date||''}</td><td>${l.description||''}</td><td>${(l.debit||0).toFixed(2)}</td><td>${(l.credit||0).toFixed(2)}</td><td style="font-weight:bold;color:${l.balance>=0?'green':'red'}">${(l.balance||0).toFixed(2)}</td></tr>`);
        html += '</table>'; document.getElementById('stmt-result').innerHTML = html;
...
async function loadSupplierStatementForm() {
  try {
    ...
    document.getElementById('tab-content').innerHTML = `<div class="card"><button class="btn-secondary" onclick="loadReports()">🔙 رجوع</button><h3>كشف حساب مورد</h3><select id="stmt-supp" class="input-field">${opts}</select><button id="btn-stmt-supp" class="btn-primary">عرض الكشف</button><div id="stmt-result"></div></div>`;
...
(function enableTabDragAndDrop() {
  const nav = document.querySelector('nav'); if (!nav) return; let dragged = null;
...
document.addEventListener('click', e => {
  if (e.target.classList.contains('tab')) {
    ...
    if (tab==='dashboard') loadDashboard(); else if (tab==='items') loadItems(); else if (tab==='sale-invoice') loadSaleInvoiceForm(); else if (tab==='purchase-invoice') loadPurchaseInvoiceForm(); else if (tab==='customers') loadCustomers(); else if (tab==='suppliers') loadSuppliers(); else if (tab==='categories') loadCategories(); else if (tab==='payments') loadPayments(); else if (tab==='invoices') loadInvoices(); else if (tab==='reports') loadReports();
  }
});
async function verifyUser() {
  try {
    const data = await apiCall('/verify', 'POST');
    if (data.verified) { document.getElementById('user-name').textContent = user.first_name; document.getElementById('loading').style.display = 'none'; document.getElementById('main').style.display = 'block'; loadDashboard(); }
