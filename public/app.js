...
async function loadDashboard() {
  try {
    const [items, customers, suppliers, invoices, monthly, daily] = await Promise.all([
      apiCall('/items', 'GET'), apiCall('/entities/customers', 'GET'), apiCall('/entities/suppliers', 'GET'), apiCall('/invoices', 'GET'),
...
async function loadCustomers() {
  try {
    const customers = await apiCall('/entities/customers', 'GET'); customersCache = customers;
...
function showAddCustomerModal() {
  ...
  document.getElementById('save-add-cust').onclick = async () => {
    ...
    try { await apiCall('/entities/customers', 'POST', { name: n, phone: document.getElementById('add-cust-phone').value.trim(), address: document.getElementById('add-cust-address').value.trim() }); document.body.removeChild(overlay); loadCustomers(); } catch (e) { alert('خطأ: '+e.message); }
  };
...
function showEditCustomerModal(custId) {
  ...
  document.getElementById('save-cust-edit').onclick = async () => {
    ...
    try { await apiCall('/entities/customers', 'PUT', { id: custId, name: n, phone: document.getElementById('edit-cust-phone').value.trim(), address: document.getElementById('edit-cust-address').value.trim() }); document.body.removeChild(overlay); loadCustomers(); } catch (e) { alert('خطأ: '+e.message); }
  };
...
async function loadSuppliers() {
  try {
    const suppliers = await apiCall('/entities/suppliers', 'GET'); suppliersCache = suppliers;
...
function showAddSupplierModal() {
  ...
  document.getElementById('save-add-sup').onclick = async () => {
    ...
    try { await apiCall('/entities/suppliers', 'POST', { name: n, phone: document.getElementById('add-sup-phone').value.trim(), address: document.getElementById('add-sup-address').value.trim() }); document.body.removeChild(overlay); loadSuppliers(); } catch (e) { alert('خطأ: '+e.message); }
  };
...
function showEditSupplierModal(supId) {
  ...
  document.getElementById('save-sup-edit').onclick = async () => {
    ...
    try { await apiCall('/entities/suppliers', 'PUT', { id: supId, name: n, phone: document.getElementById('edit-sup-phone').value.trim(), address: document.getElementById('edit-sup-address').value.trim() }); document.body.removeChild(overlay); loadSuppliers(); } catch (e) { alert('خطأ: '+e.message); }
  };
...
async function loadInvoiceFormByType(type) {
  try {
    const [customers, suppliers, items] = await Promise.all([apiCall('/entities/customers','GET'),apiCall('/entities/suppliers','GET'),apiCall('/items','GET')]);
...
async function loadInvoices() {
  try {
    ...
    customersCache = (await apiCall('/entities/customers','GET')); suppliersCache = (await apiCall('/entities/suppliers','GET')); itemsCache = (await apiCall('/items','GET'));
...
async function deleteCustomer(id) { if (!await confirmDialog('متأكد من حذف العميل؟')) return; try { await apiCall(`/entities/customers?id=${id}`,'DELETE'); alert('تم الحذف'); loadCustomers(); } catch(e) { alert('خطأ: '+e.message); } }
async function deleteSupplier(id) { if (!await confirmDialog('متأكد من حذف المورد؟')) return; try { await apiCall(`/entities/suppliers?id=${id}`,'DELETE'); alert('تم الحذف'); loadSuppliers(); } catch(e) { alert('خطأ: '+e.message); } }
...
async function loadPayments() {
  try {
    const [payments, invoices, customers, suppliers] = await Promise.all([apiCall('/payments','GET'), apiCall('/invoices','GET'), apiCall('/entities/customers','GET'), apiCall('/entities/suppliers','GET')]);
